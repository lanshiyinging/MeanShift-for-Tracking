...
%%%%%%%%%%%%%%%%%%根据一幅目标全可见的图像圈定跟踪目标%%%%%%%%%%%%%%%%%%%%%%%
g_rect=load('DragonBaby/groundtruth_rect.txt');

dataset_path='DragonBaby/img/';
frameFile=dir('DragonBaby/img/*.jpg');

frameNum=length(frameFile);

I = imread([dataset_path, frameFile(1).name]);
figure(1);
imshow(I);
...
for l=2:frameNum
    Im=imread([dataset_path, frameFile(l).name]);
    num=0;
    Y=[2,2];
    ...
    title('目标跟踪结果及其运动轨迹');
    hold on;
    lot([v1,v1+v3],[v2,v2],[v1,v1],[v2,v2+v4],[v1,v1+v3],[v2+v4,v2+v4],[v1+v3,v1+v3],[v2,v2+v4],'LineWidth',2,'Color','r');
